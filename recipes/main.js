...
function displayRecipes(recipes){
    ...
    recipes.forEach(recipe=>{
        ...
        const recipeTags = recipe.tags.map(tag =>`<span class="tags">${tag}</span>`).join("");
...
form.addEventListener("submit", function(e){
    ...
    const searchRecipe = searchInput.value.toLowerCase();
    const filtered = recipes.filter(recipe =>
        recipe.name.toLowerCase().includes(searchRecipe) ||
        recipe.description.toLowerCase().includes(searchRecipe) ||
        recipe.tags.join(" ").toLowerCase().includes(searchRecipe)
    );
